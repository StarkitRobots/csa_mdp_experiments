...
void BallApproach::updateLimits()
{
  ...
  state_limits << 0, max_dist, -M_PI, M_PI, -M_PI, M_PI, min_step_x, max_step_x, -max_step_y, max_step_y,
      -max_step_theta, max_step_theta;
  action_limits << -max_step_x_diff, max_step_x_diff, -max_step_y_diff, max_step_y_diff, -max_step_theta_diff,
      max_step_theta_diff;
...
double BallApproach::getReward(const Eigen::VectorXd& state, const Eigen::VectorXd& action,
                               const Eigen::VectorXd& dst) const
{
  ...
  if (!seeBall(dst))
    reward += no_view_reward;
  return reward;
}

Problem::Result BallApproach::getSuccessor(const Eigen::VectorXd& state, const Eigen::VectorXd& action,
...
Eigen::VectorXd BallApproach::getStartingState(std::default_random_engine* engine) const
{
  ...
  std::uniform_real_distribution<double> viewable_distrib(-viewing_angle, viewing_angle);
