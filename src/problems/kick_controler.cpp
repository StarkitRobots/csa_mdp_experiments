...
Problem::Result KickControler::getSuccessor(const Eigen::VectorXd& state, const Eigen::VectorXd& action,
                                            std::default_random_engine* engine) const
{
  ...
  if (state.rows() != 2 + 3 * (int)players.size())
  {
    ...
    oss << "KickControler::getSuccessor: invalid dimension for state: " << state.rows() << " (expected "
        << (2 + 3 * players.size()) << ")";
  ...
  const KickOption& kick_option =
      kicker_id == -1 ? *(kick_options[kick_option_id]) : *(players[kicker_id]->kick_options[kick_option_id]);
  ...
  {
    std::ostringstream oss;
    oss << "KickControler::getSuccessor: invalid dimension for action: " << action.rows() << " (expected "
  ...
  Eigen::VectorXd kick_parameters = kdm.computeKickParameters(ball_seen, decision_actions);
  ...
  {
    const Eigen::Vector3d& kicker_state = getPlayerState(result.successor, kicker_id);
...
const csa_mdp::Policy& KickControler::getPolicy(int player_id, int kicker_id, int kick_option_id) const
{
  ...
  if (player_id == kicker_id)
  {
    ...
    if (ko.approach_policy)
      return *(ko.approach_policy);
  }
  // Otherwise, use custom player approach policy if provided
  if (player.approach_policy)
    return *(player.approach_policy);
  // If none policy has been found, throw an explicit error
  std::ostringstream oss;
  oss << "KickControler::getPolicy: no policy found for player " << player_id << "(kicker_id: " << kicker_id
      << " & kick_option_id: " << kick_option_id << ")";
  throw std::logic_error(oss.str());
}

void KickControler::runSteps(int max_steps, const Eigen::VectorXd& action, int kicker_id, int kick_option_id,
                             bool kicker_enabled, Problem::Result* status, std::default_random_engine* engine) const
{
  if (!simulate_approaches)
  {
  ...
  while (nb_steps <= max_steps && !reached_target[kicker_id] && !failed)
  {
    ...
    for (int player_id = 0; player_id < (int)players.size(); player_id++)
    {
      ...
      {
        if (!kicker_enabled)
          continue;  // Skip kicker if disabled
        const BallApproach& model = kick_option.approach_model;
        // Simulate approach action
        approach_status[player_id] = model.getSuccessor(approach_status[player_id].successor, pa_action, engine);
      ...
      status->successor =
          toKickControlerState(approach_status[player_id].successor, status->successor, player_id, targets[player_id]);
  ...
  {
    std::ostringstream oss;
    oss << "KickControler::performApproach: kickable state not reached after " << max_steps << std::endl
  ...
  if (kicker_enabled)
    status->reward -= nb_steps / (2 * walk_frequency);
}

double KickControler::getApproximatedTime(const Eigen::VectorXd& src, const Eigen::VectorXd& dst) const
{
...
void KickControler::approximateKickerApproach(const Eigen::Vector2d& ball_real_pos, const Eigen::VectorXd& action,
                                              int kicker_id, int kick_option_id, Problem::Result* status) const
{
  // Importing variables
  ...
  if (approach_steps_approximator)
  {
    ...
    // the side
    kicker_target = kick_zone.getWishedPosInField(ball_real_pos, kick_wished_dir, false);
  }
...
void KickControler::moveNonKickers(double allowed_time, const Eigen::Vector2d& ball_start,
                                   const Eigen::Vector2d& ball_end, int kicker_id, Problem::Result* status) const
{
...
void KickControler::moveRobot(double allowed_time, const Eigen::Vector2d& ball_start, const Eigen::Vector2d& ball_end,
                              int robot_id, Problem::Result* status) const
{
  // Get placing targets
...
Eigen::Vector3d KickControler::getBestTarget(const Eigen::Vector2d& ball_start, const Eigen::Vector2d& ball_end,
...
Eigen::Vector3d KickControler::getPlayerState(const Eigen::VectorXd& state, int player_id) const
{
  return state.segment(2 + 3 * player_id, 3);
}

Eigen::Vector3d KickControler::getTarget(const Eigen::VectorXd& state, const Eigen::VectorXd& action, int player_id,
                                         int kicker_id, int kick_option) const
{
  ...
  {
    throw std::logic_error("KickControler::getTarget: only single kick types are supported");
  }
...
Eigen::VectorXd KickControler::toKickControlerState(const Eigen::VectorXd& pa_state,
                                                    const Eigen::VectorXd& prev_kc_state, int player_id,
                                                    const Eigen::Vector3d& target) const
{
  // Getting basic data
...
void KickControler::analyzeActionId(int action_id, int* kicker_id, int* kick_id) const
{
  ...
  {
    std::ostringstream oss;
    oss << "KickControler::getSuccessor: invalid value for action_id: " << action_id << " (expecting value in [" << 0
        << "," << cpt << "[)";
    throw std::logic_error(oss.str());
  }
}

const std::vector<std::string>& KickControler::getAllowedKicks(int kicker_id, int kick_option)
{
  return players[kicker_id]->kick_options[kick_option]->kick_model_names;
}

double KickControler::getKickDir(const Eigen::VectorXd& state, const Eigen::VectorXd& action) const
{
  int action_id = action(0);
  int kicker_id(0), kick_option(0);
  analyzeActionId(action_id, &kicker_id, &kick_option);
  // Importing basic kick properties
  const KickDecisionModel& kdm = *(players[kicker_id]->kick_options[kick_option]->kick_decision_model);
  int kick_dims = kdm.getActionsLimits().rows();
  // Renaming variables to improve readability
  Eigen::Vector2d ball_seen = state.segment(0, 2);
...
Eigen::Matrix<double, 2, 2> KickControler::getFieldLimits() const
{
  ...
  field_limits << -field_length / 2, field_length / 2, -field_width / 2, field_width / 2;
  return field_limits;
}
...
Eigen::Matrix<double, 3, 2> KickControler::getPlayerLimits() const
{
  ...
  player_limits << -field_length / 2, field_length / 2, -field_width / 2, field_width / 2, -M_PI, M_PI;
...
void KickControler::updateActionsLimits()
{
  ...
  for (size_t p_id = 0; p_id < players.size(); p_id++)
  {
    ...
    {
      // kick_model
      const KickDecisionModel& kdm = *(players[p_id]->kick_options[ko_id]->kick_decision_model);
...
void KickControler::initialBallNoise(double ball_x, double ball_y, double* ball_real_x, double* ball_real_y,
                                     std::default_random_engine* engine) const
{
...
void KickControler::moveBall(double src_x, double src_y, double* dst_x, double* dst_y, bool* terminal,
                             double* reward) const
{
...
bool KickControler::isCollidingGoalie(double src_x, double src_y, double* dst_x, double* dst_y) const
{
  ...
  if (below_x || above_x || below_y || above_y)
    return false;
...
bool KickControler::isGoal(double src_x, double src_y, double* dst_x, double* dst_y) const
{
  double dx = *dst_x - src_x;
...
bool KickControler::isOut(double src_x, double src_y, double* dst_x, double* dst_y) const
{
  (void)src_x;
  (void)src_y;
  // Computing limits
