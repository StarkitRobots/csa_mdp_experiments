...
SSLBallApproach::SSLBallApproach()
  :  // State limits
  max_dist(1.0)
  ,  // Roughly OK, hard to have more ideas
  max_speed(0.8)
...
void SSLBallApproach::updateLimits()
{
  ...
  state_limits << 0, max_dist, -M_PI, M_PI, -M_PI, M_PI, -max_speed, max_speed, -max_speed, max_speed, -max_speed_theta,
      max_speed_theta;
...
double SSLBallApproach::getReward(const Eigen::VectorXd& state, const Eigen::VectorXd& action,
...
Problem::Result SSLBallApproach::getSuccessor(const Eigen::VectorXd& state, const Eigen::VectorXd& action,
...
bool SSLBallApproach::isKickable(const Eigen::VectorXd& state) const
{
  ...
  return (ball_x >= 0 && ball_x <= kick_x_max) && (ball_y >= -kick_y_tol && ball_y <= kick_y_tol) &&
         (kick_dir >= -kick_dir_tol && kick_dir <= kick_dir_tol) && (speed_x >= 0 && speed_x <= kick_speed_x_max) &&
