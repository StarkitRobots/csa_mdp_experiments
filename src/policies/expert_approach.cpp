...
Eigen::VectorXd ExpertApproach::getConfig() const
{
  ...
  params << max_rotate_radius, min_far_radius, radius, step_p, far_theta_p, rotate_theta_p, rotate_lateral_p,
      near_theta_p, near_lateral_p, stop_y_near, max_y_near, wished_x, wished_y, target_theta_tol, ball_theta_tol;
  return params;
}
void ExpertApproach::setConfig(Type newType, const Eigen::VectorXd& params)
{
  ...
  {
    throw std::logic_error("ExpertApproach::setConfig Invalid parameters size");
  }
  type = newType;
...
Eigen::VectorXd ExpertApproach::getRawAction(const Eigen::VectorXd& state, State* final_state) const
{
  ...
  if ((approach_type == "opportunist" && std::fabs(target_angle) > lateral_threshold) || approach_type == "lateral")
  {
...
void ExpertApproach::fromJson(const Json::Value& v, const std::string& dir_name)
{
  ...
  if (!valid_type)
  {
    throw std::runtime_error("ExpertApproach::fromJson: invalid approach_type: '" + approach_type + "'");
  }
  if (foot_y_offset < 0)
  {
...
std::unique_ptr<FATree> ExpertApproach::extractFATree() const
{
  ...
  throw std::logic_error("ExpertApproach::extractFATree: unknown type: '" + approach_type + "'");
}
...
std::unique_ptr<FATree> ExpertApproach::extractClassicFATree() const
{
  ...
  std::unique_ptr<FunctionApproximator> rotate_node(new LinearApproximator(rotate_bias, rotate_coeffs));
...
std::unique_ptr<FATree> ExpertApproach::extractLateralFATree() const
{
  ...
  // Build approximators
  std::unique_ptr<FunctionApproximator> far_node(new LinearApproximator(far_bias, far_coeffs));
  std::unique_ptr<FunctionApproximator> rotate_left_node(new LinearApproximator(rotate_left_bias, rotate_coeffs));
  std::unique_ptr<FunctionApproximator> rotate_right_node(new LinearApproximator(rotate_right_bias, rotate_coeffs));
  std::unique_ptr<FunctionApproximator> near_right_node(new LinearApproximator(near_right_bias, near_coeffs));
  std::unique_ptr<FunctionApproximator> near_left_node(new LinearApproximator(near_left_bias, near_coeffs));
  ...
  std::unique_ptr<Split> ball_align_split_low(new OrthogonalSplit(1, -ball_theta_tol));
  std::unique_ptr<Split> ball_align_split_high(new OrthogonalSplit(1, ball_theta_tol));
  std::unique_ptr<Split> target_align_right_split_low(new OrthogonalSplit(2, -theta_bias - target_theta_tol));
  std::unique_ptr<Split> target_align_right_split_high(new OrthogonalSplit(2, -theta_bias + target_theta_tol));
  std::unique_ptr<Split> target_align_left_split_low(new OrthogonalSplit(2, theta_bias - target_theta_tol));
  std::unique_ptr<Split> target_align_left_split_high(new OrthogonalSplit(2, theta_bias + target_theta_tol));
  // Building complete tree
  std::vector<std::unique_ptr<FunctionApproximator>> approximators;
...
std::string to_string(ExpertApproach::State state)
{
  switch (state)
  {
    case ExpertApproach::State::near:
      return "near";
    case ExpertApproach::State::far:
      return "far";
    case ExpertApproach::State::rotate:
      return "rotate";
  }
...
std::string to_string(ExpertApproach::Type type)
{
  switch (type)
  {
    case ExpertApproach::Type::cartesian:
      return "cartesian";
    case ExpertApproach::Type::polar:
      return "polar";
  }
